...
public class TimedKTailTest_WithIncrementalInference {

	@Parameters(name = "{index}: strategy:{0}; useCaching:{1}")
    public static Collection<Object[]> data() {
    	ArrayList<Object[]> objs = new ArrayList<>();
    	
    	for ( MergeStrategies m : MergeStrategies.values() ){
    		if ( m == MergeStrategies.LAYERS_BACKWARD ){
    			continue;
    		}
    		for ( boolean useCaching : TestUtils.TF ){
    			objs.add(new Object[]{ m, useCaching } );
    		}
    	}
	...
	public void setUp() throws Exception {
    	...
    	poli.setUseIncrementalMerging(true);
	}
